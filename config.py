...
# نکته: بعضی این فیدها (مخصوصا Everyday Grammar خودِ VOA) خیلی پرکار نیستن (گاهی ماه‌ها
# بین دو پست فاصله می‌افته)؛ به همین خاطر (نگاه کن به fetch_content.py) هیچ فیلتر سن‌محوری
# روی نمایش نیست - حتی پست چندماهه‌ی این منابع هم نشون داده می‌شه، نه اینکه بخش خالی بمونه.
# ---------------------------------------------------------------------------
RSS_SOURCES = {
    "گرامر": [
        {
...
# ---------------------------------------------------------------------------
# نکته: برخلاف سایت خبری، اینجا هیچ سقف سن‌محوری روی نمایش آیتم‌های واقعی نیست (نگاه کن
# به توضیح داخل fetch_content.fetch_all) - چون منابع آموزش زبان گاهی ماه‌ها بین دو پست
# فاصله می‌افتن و یک نکته‌ی گرامری/نوشتاری با گذشت زمان "کهنه" نمی‌شه.
