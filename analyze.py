...
def summarize_section(section: str, items: list) -> list:
    ...
    - explain_fa: ۱-۳ جمله فارسی که می‌گه این درس دقیقا چه نکته‌ای از انگلیسی رو یاد می‌ده.
    ...
    system_prompt = (
        "تو معلم زبان انگلیسی هستی که برای فارسی‌زبان‌هایی که دارن انگلیسی یاد می‌گیرن محتوا "
        "آماده می‌کنی. برای هر مطلب داده‌شده (که از یک منبع واقعی آموزش زبان مثل VOA Learning "
        "English یا Merriam-Webster گرفته شده) دو کار می‌کنی:\n"
        "۱) عنوان رو به فارسیِ روان (نه ترجمه لغت‌به‌لغت خشک) برگردون - طوری که یک زبان‌آموز "
        "فارسی‌زبان فورا بفهمه این مطلب درباره چیه.\n"
        "۲) یک توضیح ۱ تا ۳ جمله‌ای فارسی بنویس که دقیقا بگه این مطلب چه نکته‌ی زبانی/گرامری/"
        "واژگانی رو آموزش می‌ده و چرا یادگیریش مفیده - نه فقط خلاصه‌ی خبری از متن. اگه مطلب "
        "یک واژه یا اصطلاح خاصه، حتما معنی فارسی همون کلمه/اصطلاح رو هم توی توضیح بیار.\n\n"
        "خروجی رو دقیقا به فرمت JSON زیر بده، بدون هیچ متن اضافه قبل یا بعدش:\n"
        '{"items": [{"title_fa": "...", "explain_fa": "..."}, ...]}\n'
    ...
    try:
        raw = _call_llm(system_prompt, user_prompt, max_tokens=2500)
        ...
        for it, r in zip(items, results):
            ...
            explain_fa = (r or {}).get("explain_fa", "").strip()
            if title_fa:
                it["title_fa"] = title_fa
            if explain_fa:
                it["explain_fa"] = explain_fa
    except Exception as e:
